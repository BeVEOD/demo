/*
  Lightweight JavaScript calendar control v1.0.1
  ...
  USAGE:
    ...
    options.format: specifies date display format using  %d, %e, %b, %m, %B, %y, %Y
                    date format specifiers (default is '%d %b %Y').

    options.images_dir: calendar prev, next images directory (default is '.').

    options.month_names: array of 12 month names (defaults to english).
...
var PREV_MONTH_IMAGE = 'std/calendar_prev.png'
var NEXT_MONTH_IMAGE = 'std/calendar_next.png'
...
function calendar_update() {
  ...
  get_element('calendar_prev').onclick = calendar_prev;
  get_element('calendar_next').onclick = calendar_next;
...
function calendar_open(input_element, options) {
  ...
  // Set up previous and next month images.
  var images = calendar.element.getElementsByTagName('img');
  if (images.length == 0) {
    get_element('calendar_prev').appendChild(document.createElement('img'));
    get_element('calendar_next').appendChild(document.createElement('img'));
    images = calendar.element.getElementsByTagName('img');
  }
  images[0].src = images_dir + '/' + PREV_MONTH_IMAGE;
  images[1].src = images_dir + '/' + NEXT_MONTH_IMAGE;
...
// Header row.
document.write('<tr><td id="calendar_prev"></td>');
document.write('<td id="calendar_header" colspan="5"></td>');
document.write('<td id="calendar_next"></td></tr>');
// Day letters row.
document.write('<tr>')
var day_letters = 'SMTWTFS';
for (var i=0; i < day_letters.length; i++) {
  document.write('<td class="day_letter">' + day_letters.substr(i,1) + '</td>');
}
document.write('</tr>');
// Day numbers rows.
