...
for (var t=0; t<ultags.length; t++){
	...
	ultags[t].parentNode.onmouseover=function() {
		$(this.getElementsByTagName("ul")[0]).css('visibility', "visible").css('zIndex', 99999999);
	}
...
for (sid in hf) {
  for (op in hf[sid]) {
    ...
    var myop =  $(f).find('[data-field="' + sid + '"] OPTION[value="' + op + '"]');
    var comp = myop ? myop.parent() : null;
    var lbl  = comp.parent('LABEL, UL');
    var val = hf[sid][op];
	...
	if (comp.hasClass('date')) {
	  ...
	  fld.val(fr_date)
	    ...
	    var format = fld.datepicker("option", "dateFormat");
    	    fld.datepicker("option", "dateFormat", 'yy-mm-dd');

	    fld.datepicker("option", "dateFormat", format);
...
var res = "";
var inp = $(lab).contents('INPUT.calendar');
if (inp.length > 0) {
  var f = inp.datepicker("option", "dateFormat");
  inp.datepicker("option", "dateFormat", 'yy-mm-dd');
  res = inp.datepicker().val();
  inp.datepicker("option", "dateFormat", f);
  return res;
}
