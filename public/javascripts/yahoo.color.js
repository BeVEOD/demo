...
Control.ColorPicker.prototype = {
	...
	this.openOnFocusListener = this.open.bindAsEventListener(this);
	this.closeOnBlurListener = this.close.bindAsEventListener(this);
	Event.observe(this.swatch, "focus", this.openOnFocusListener);
	Event.observe(this.swatch, "blur", this.closeOnBlurListener);
	...
	if (!YAHOO.util.Color.isValidRGB(rgb)) return;
	(this.swatch || this.field).style.backgroundColor = "rgb(" + rgb[0] + ", " + rgb[1] + ", " + rgb[2] + ")";
	var hsv = YAHOO.util.Color.rgb2hsv( rgb[0], rgb[1], rgb[2] );
	(this.swatch || this.field).style.color = (hsv[2] > 0.65) ? "#000000" : "#FFFFFF";
    },
    update : function(x, y) {
