...
## It should be placed at 'app/controllers/users_controller.rb'
...
class UsersController < ApplicationController
  ...
  def linker
    ...
    when :customers
      val = params["customer"] ? params["customer"]["0"] : {}
      @customers = Customer.search(val, params.reject{|k,v| !k[/comp_/]}, :page => params[:page], :per_page => params[:per_page])
      render :partial => '/customers/list_for_linker', :locals => {:assoc_sid => :user, :assoc_type => :has_many}
    when :user_accesses
      ...
      render :partial => '/user_accesses/list_for_linker', :locals => {:assoc_sid => :user, :assoc_type => :has_many}
    when :accesses
      ...
      render :partial => '/accesses/list_for_linker', :locals => {:assoc_sid => :users, :assoc_type => :has_many_through}
    when :profiles
      ...
      render :partial => '/profiles/list_for_linker', :locals => {:assoc_sid => :users, :assoc_type => :has_and_belongs_to_many}
    when :dev_feedbacks
      val = params["dev_feedback"] ? params["dev_feedback"]["0"] : {}
      @dev_feedbacks = DevFeedback.search(val, params.reject{|k,v| !k[/comp_/]}, :page => params[:page], :per_page => params[:per_page])
      render :partial => '/dev_feedbacks/list_for_linker', :locals => {:assoc_sid => :user, :assoc_type => :has_many}
    when :file_imports
      val = params["file_import"] ? params["file_import"]["0"] : {}
      @file_imports = FileImport.search(val, params.reject{|k,v| !k[/comp_/]}, :page => params[:page], :per_page => params[:per_page])
      render :partial => '/file_imports/list_for_linker', :locals => {:assoc_sid => :user, :assoc_type => :has_many}
    end
  end
  ...
  def report_chooser
    ...
    @data_sets = SmartQuery.all(:conditions => {:table_sid => ['users', '']})
    m_par = params[:users] || {}
  ...
  def report_check_params
    ...
    @x_type = User::FIELDS[params[:x_field]]
    ...
    if params[:y_field]
      @y_type = User::FIELDS[params[:y_field]]
    ...
    o = params["serie_1"][:operation] ||= [:integer, :float, :double].include?(User::FIELDS[params["serie_1"][:field]]) ? :value : :count

    # TODO manage field is a model_method

    render(:text => "No correct serie[:field] parameter") and return if !User::FIELDS.has_key?(f) && (o != 'count')
    render(:text => "No correct serie[:operation] parameter") and return if (![:integer, :float, :double, :boolean, :static_list].include?(User::FIELDS[f]) && params[:serie_1][:operation].blank?)
    (2..5).each {|i|
      break if params["serie_#{i}"].blank? || !params["serie_#{i}"].is_a?(Hash)
      f = params["serie_#{i}"][:field]
      render(:text => "No correct serie_#{i}[:field] parameter") and return if f.blank? || !User::FIELDS.has_key?(f)
      render(:text => "No correct serie_#{i}[:operation] parameter") and return if (![:integer, :float, :double, :boolean, :static_list].include?(User::FIELDS[f]) && params["serie_#{i}"][:operation].blank?)
    }
  ...
  def report_load_datasets
    ...
    if @y_field
      case @y_type
      when :string
        @y_labels = User.all(:select => User.connection.distinct(@y_field, nil)).map{|r| r.send(@y_field)}
      when :static_list
        @y_labels = User.const_get(@y_field.pluralize.upcase)
      when :ho_assoc, :bt_assoc, :habtm_assoc
        @y_labels = User.all(:include => @y_field).map{|r| r.send(@y_field)}
      end
      while(params["serie_#{i}"]) do
        ...
        @y_labels.each_with_index { |l,idx|
          ...
          case @y_type
          when :string
            ds["values_for_#{l}"] = User.report_calculation(params["serie_#{i}"][:operation],
          ...
          when :bt_assoc
            fk = User.reflect_on_association(@y_field.to_sym).primary_key_name

            #        User.send(:with_scope, :find => {:conditions => {fk => l.id}}) {
            #        ds["values_for_#{l.id}"] = report_retrieve_data(params["serie_#{i}"][:operation].to_s, params["serie_#{i}"][:field])

            ds["values_for_#{l.id}"] = User.report_calculation(params["serie_#{i}"][:operation],
            params["serie_#{i}"][:field],
    ...
    while(params["serie_#{i}"]) do
      ds = {:values => User.report_calculation(params["serie_#{i}"][:operation],
        params["serie_#{i}"][:field],
  ...
  def update
    ...
    @user_attributes = params[:user] ? params[:user][params[:id]].clone : {}
    @user_attributes.merge!(:id => params[:id]) if @user_attributes
    @user_attributes.delete(:active) if @user_attributes && restrict_active_for_update_5
    @user_attributes.delete(:password) if @user_attributes && restrict_password_for_update_5
    @user_attributes.delete(:hashed_password) if @user_attributes && restrict_hashed_password_for_update_5
    @user_attributes.delete(:salt) if @user_attributes && restrict_salt_for_update_5
    @user_attributes.delete(:last_login) if @user_attributes && restrict_last_login_for_update_5
    @user_attributes.delete(:last_session_id) if @user_attributes && restrict_last_session_id_for_update_5
    @user_attributes.delete(:user_accesses) if @user_attributes && restrict_user_accesses_for_update_5
    @user_attributes.delete(:accesses) if @user_attributes && restrict_accesses_for_update_5
    @user_attributes.delete(:profiles) if @user_attributes && restrict_profiles_for_update_5
    @user = User.load_from_params(@user_attributes)
    if @user.nil?
      flash[:warning] ||= []
      flash[:warning] << _("Warning: %{obj} with ID %{id} does not exist!") % {:obj => 'user', :id => params[:id]}
    ...
    if @user.save
      flash[:notice] = _("%{page} was successfully updated.") % {:page => _("User")}
    else
      if request.xhr?
        render(:json => {:errors =>
    ...
    if params[:user].length == 1 # single edit
  ...
  def search
    ...
    @user = User.load_from_params(params[:user]["0"]) if params[:user]
    @user ||= User.new
    page	   = params[:page].to_i > 0 ? params[:page].to_i : 1
    per_page   = params[:per_page]
    per_page ||= User.count if params[:format] && %w(xml xls csv).include?(params[:format])
    per_page   = 50 if !per_page || per_page == 0

    # GET RESULTS FROM SMART_QUERY OR PARAMS

    if params[:user]
      my_p = params[:user]["0"].reject{|k, v|
        params["comp_#{k}"].blank? || (v.blank? && !params["comp_#{k}"].include?('NULL'))
      } if params[:user]["0"]
      @users = User.search(my_p, params.reject{|k,v| !k[/comp_/]}, :page => page, :per_page => per_page)
    elsif params[:query]
      @users = User.active_filter(ActiveSupport::JSON.decode(params[:query])).paginate(:page => page, :per_page => per_page)
    elsif params[:fts_query]
      l = params[:limit] ? params[:limit] : :all
      @users = User.find_with_ferret(params[:fts_query], :limit => l)
      ...
      @users = User.search(@smart_query, :page => page, :per_page => per_page)
    else
      opts = {:include => [:customers,:profiles,:dev_feedbacks]}
      opts[:page]		  = page
      opts[:per_page]	  = per_page
      opts[:order]		  = @default_order
      opts[:order]		||= 'users.' +params[:sort_by] if !params[:sort_by].blank?
    ...
    if !@users.blank? && params[:format] # GET
      if params[:format] == 'xml'
  ...
  def feed
    ...
    respond_to do |format|
      format.rss {render :template => 'users/rss', :layout => false}
      format.xml {render :template => 'users/rss', :layout => false}
      format.html {render :template => 'users/rss', :layout => false}
      format.atom {render :template => 'users/atom', :layout => false}
    end
  end
  ...
  # Author:: Sylvain Abélard
  # Version:: 21
  # Last Update:: 2013-03-20 15:36:31 UTC
  # Status:: Validation Pending
  ...
  def login
    ...
    if request.get?
      if (@current_user && @current_user.has_profile?('Logged In'))
    ...
    if flash[:login_succeeded]
      flash[:notice] = _("You're now logged as %{login}") % {:login => @current_user.login}
      return_to = session[:return_to]
      if return_to
        session[:return_to] = nil
        redirect_to(return_to)
      elsif
        params[:controller] == 'users' && ['login','logout'].include?(params[:action])
    ...
    flash[:warning] = _("Connection failed - wrong login or password")
    render :action => :login, :layout => 'login'
  end
  ...
  # Author:: Sylvain Abélard
  # Version:: 4
  # Last Update:: 2010-06-16 12:08:38 UTC
  # Status:: Validation Pending
  ...
  # Author:: Yann Azoury
  # Version:: 1
  # Last Update:: 2009-09-11 11:53:03 UTC
  # Status:: Validation Pending
  ...
  # Rights management: access graph.
  #
  # Author:: Sylvain Abélard
  # Version:: 5
  # Last Update:: 2010-06-16 12:39:37 UTC
  # Status:: Requires Tests
  ...
  def access_graph
    ...
    @user = User.find_by_id(params[:id])
    if params[:format] && @user
      @user_accesses = @user.accesses
      @profile_accesses = @user.profiles.map(&:accesses).flatten.uniq
      @graph = render_graph('users/access_graph.dot', params[:format], 'fdp')
  ...
  # Author:: Yann Azoury
  # Version:: 5
  # Last Update:: 2012-04-22 01:27:41 UTC
  # Status:: Requires Tests
  ...
  def field_access_graph
    ...
    @user = User.find_by_id(params[:id])
    if params[:format] && @user
      @user_accesses = @user.accesses.all(:conditions => ["field_sid IS NOT NULL AND (user_accesses.write = ? OR user_accesses.read = ?)", true, true])
      @profile_accesses = []
      @user.profiles.each do |p|
        @profile_accesses += p.accesses.all(:conditions => ["field_sid IS NOT NULL AND (profile_accesses.write = ? OR profile_accesses.read = ?)", true, true])
      end
      @profile_accesses.uniq!
      @graph = render_graph('users/field_access_graph.dot', params[:format], 'fdp')
      send_data @graph, :disposition => 'inline'
    end
  end
  ...
  # Author:: Yann Azoury
  # Version:: 6
  # Last Update:: 2011-03-29 19:39:12 UTC
  # Status:: Validation Pending
  ...
  # Uses PDFKit to generate a PDF file.
  #
  # Author:: Yann Azoury
  # Version:: 3
  # Last Update:: 2013-07-09 15:39:59 UTC
  # Status:: Validation Pending
  ...
  def restrict_active_for_create_5
  ...
  def restrict_active_for_show_5
  ...
  def restrict_active_for_edit_5
  ...
  def restrict_active_for_update_5
  ...
  def restrict_password_for_create_5
  ...
  def restrict_password_for_show_5
  ...
  def restrict_password_for_edit_5
  ...
  def restrict_password_for_update_5
  ...
  # For field hashed_password


  def restrict_hashed_password_for_create_5
  ...
  def restrict_hashed_password_for_show_5
  ...
  def restrict_hashed_password_for_edit_5
  ...
  def restrict_hashed_password_for_update_5
  ...
  def restrict_salt_for_create_5
  ...
  def restrict_salt_for_update_5
  ...
  # For field last_login


  def restrict_last_login_for_create_5
  ...
  def restrict_last_login_for_show_5
  ...
  def restrict_last_login_for_edit_5
  ...
  def restrict_last_login_for_update_5
  ...
  # For field last_session_id


  def restrict_last_session_id_for_create_5
  ...
  def restrict_last_session_id_for_show_5
  ...
  def restrict_last_session_id_for_edit_5
  ...
  def restrict_last_session_id_for_update_5
  ...
  # For field user_accesses


  def restrict_user_accesses_for_create_5
  ...
  def restrict_user_accesses_for_show_5
  ...
  def restrict_user_accesses_for_edit_5
  ...
  def restrict_user_accesses_for_update_5
  ...
  def restrict_accesses_for_create_5
  ...
  def restrict_accesses_for_show_5
  ...
  def restrict_accesses_for_edit_5
  ...
  def restrict_accesses_for_update_5
  ...
  def restrict_profiles_for_create_5
  ...
  def restrict_profiles_for_show_5
  ...
  def restrict_profiles_for_edit_5
  ...
  def restrict_profiles_for_update_5
  ...
  def faveod_cookies_loading
    @pres_mode = cookies["#{APP_SID}_users_pres_mode"].to_sym unless cookies["#{APP_SID}_users_pres_mode"].blank?
    @pres_mode = params["#{APP_SID}_users_pres_mode"].to_sym unless params["#{APP_SID}_users_pres_mode"].blank?
  end
